...
-- @note Modified to use libknot.so and libkres.so
...
local libkres = ffi.load('kres')
loadfile('/usr/lib/knot-resolver/kres-gen.lua')
...
ffi.metatype( sockaddr_t, {
	__index = {
		len = function(sa) return libkres.kr_inaddr_len(sa) end,
		ip = function (sa) return libkres.kr_inaddr(sa) end,
		family = function (sa) return libkres.kr_inaddr_family(sa) end,
		port = function (sa) return libkres.kr_inaddr_port(sa) end,
	},
	__tostring = function(sa)
		assert(ffi.istype(sockaddr_t, sa))
		local len = ffi.new('size_t[1]', str_addr_buf_len)
		local ret = libkres.kr_inaddr_str(sa, str_addr_buf, len)
		if ret ~= 0 then
...
local function dname2str(dname)
	...
	local text_name = ffi.gc(knot.knot_dname_to_str(nil, dname, 0), C.free)
	...
	__index = {
		...
		tostring = function(rr, i)
			...
			if rr:rdcount() > 0 then
				local ret
				if i ~= nil then
					ret = knot.knot_rrset_txt_dump_data(rr, i, rrset_buf, rrset_buflen, libkres.KR_DUMP_STYLE_DEFAULT)
				else
					ret = -1
				end
		...
		txt_dump = function(rr, style)
			...
			local ret = knot.knot_rrset_txt_dump(rr, dump, size,
							style or libkres.KR_DUMP_STYLE_DEFAULT)
			local result = nil
			if ret >= 0 then
		...
		type_covered = function(rr, i)
			...
			return tonumber(libkres.kr_rrsig_type_covered(knot.knot_rdataset_at(rr.rrs, i)))
		end,
...
ffi.metatype( knot_pkt_t, {
	__new = function (_, size, wire)
		...
		if wire == nil then
			libkres.kr_rnd_buffered(pkt.wire, 2) -- randomize the query ID
		else
			assert(size <= #wire)
	...
	__index = {
		...
		dobit = function(pkt, val)
			...
			assert(ffi.istype(knot_pkt_t, pkt))
			return libkres.kr_pkt_has_dnssec(pkt)
		end,
		-- Question
		qname = function(pkt)
			assert(ffi.istype(knot_pkt_t, pkt))
		...
		qclass = function(pkt)
			assert(ffi.istype(knot_pkt_t, pkt))
			return libkres.kr_pkt_qclass(pkt)
		end,
		qtype  = function(pkt)
			assert(ffi.istype(knot_pkt_t, pkt))
			return libkres.kr_pkt_qtype(pkt)
		end,
		...
		put = function (pkt, owner, ttl, rclass, rtype, rdata)
			assert(ffi.istype(knot_pkt_t, pkt))
			local ret = libkres.kr_pkt_put(pkt, owner, ttl, rclass, rtype, rdata, #rdata)
			if ret ~= 0 then return nil, knot_error_t(ret) end
			return true
		end,
		...
		put_rr = function (pkt, rr, rotate, flags)
			assert(ffi.istype(knot_pkt_t, pkt))
			assert(ffi.istype(knot_rrset_t, rr))
			local ret = knot.knot_pkt_put_rotate(pkt, 0, rr, rotate or 0, flags or 0)
			if ret ~= 0 then return nil, knot_error_t(ret) end
			return true
		end,
		...
		has_wire = function (pkt)
			assert(ffi.istype(knot_pkt_t, pkt))
			return libkres.kr_pkt_has_wire(pkt)
		end,
		recycle = function (pkt)
			assert(ffi.istype(knot_pkt_t, pkt))
			local ret = libkres.kr_pkt_recycle(pkt)
			if ret ~= 0 then return nil, knot_error_t(ret) end
			return true
		end,
		clear_payload = function (pkt)
			assert(ffi.istype(knot_pkt_t, pkt))
			local ret = libkres.kr_pkt_clear_payload(pkt)
			if ret ~= 0 then return nil, knot_error_t(ret) end
			return true
		end,
		question = function(pkt, qname, qclass, qtype)
			...
			local ret = knot.knot_pkt_put_question(pkt, qname, qclass, qtype)
			if ret ~= 0 then return nil, knot_error_t(ret) end
			return true
		end,
		towire = function (pkt)
		...
		tostring = function(pkt)
			assert(ffi.istype(knot_pkt_t, pkt))
			return ffi.string(ffi.gc(libkres.kr_pkt_text(pkt), C.free))
		end,
...
ffi.metatype( kr_query_t, {
	__index = {
		...
		write = function(qry, pkt)
			assert(ffi.istype(kr_query_t, qry))
			assert(ffi.istype(knot_pkt_t, pkt))
			local ret = libkres.kr_make_query(qry, pkt)
			if ret ~= 0 then return nil, knot_error_t(ret) end
			return true
		end,
	},
...
ffi.metatype( kr_request_t, {
	__index = {
		...
		all_from_cache = function(req)
			...
			local rplan = ffi.libkres.kr_resolve_plan(req)
		...
		initial = function(req)
			assert(ffi.istype(kr_request_t, req))
			local rplan = libkres.kr_resolve_plan(req)
		...
		last = function(req)
			assert(ffi.istype(kr_request_t, req))
			local query = libkres.kr_rplan_last(libkres.kr_resolve_plan(req))
			if query == nil then return end
			return query
		end,
		resolved = function(req)
			assert(ffi.istype(kr_request_t, req))
			local qry = libkres.kr_rplan_resolved(libkres.kr_resolve_plan(req))
		...
		push = function(req, qname, qtype, qclass, flags, parent)
			...
			local rplan = libkres.kr_resolve_plan(req)
			local qry = libkres.kr_rplan_push(rplan, parent, qname, qclass, qtype)
			if qry ~= nil and flags ~= nil then
				libkres.kr_qflags_set(qry.flags, flags)
			end
			return qry
		end,
		pop = function(req, qry)
			assert(ffi.istype(kr_request_t, req))
			return libkres.kr_rplan_pop(libkres.kr_resolve_plan(req), qry)
		end,
		...
		set_extended_error = function(req, code, msg)
			...
			ffi.libkres.kr_request_set_extended_error(req, code, msg)
		end,
		...
		ensure_edns = function (req)
			assert(ffi.istype(kr_request_t, req))
			return libkres.kr_request_ensure_edns(req)
		end,
		-- Ensure that answer exists and return it; can't fail.
		ensure_answer = function (req)
			assert(ffi.istype(kr_request_t, req))
			return libkres.kr_request_ensure_answer(req)
		end,
	},
})
...
local function rank_tostring(rank)
	...
	for name, value in pairs(const_rank) do
		if ffi.libkres.kr_rank_test(rank, value) then
...
ffi.metatype( kr_cache_t, {
	__index = {
		insert = function (self, rr, rrsig, rank, timestamp)
			...
			local ret = libkres.kr_cache_insert_rr(self, rr, rrsig, tonumber(rank or 0),
											 timestamp, true)
			if ret ~= 0 then return nil, knot_error_t(ret) end
			return true
		end,
		commit = function (self)
			assert(ffi.istype(kr_cache_t, self))
			local ret = libkres.kr_cache_commit(self)
			if ret ~= 0 then return nil, knot_error_t(ret) end
			return true
		end,
	},
...
kres = {
	...
	str2dname = function(name)
		...
		local dname = ffi.gc(knot.knot_dname_from_str(nil, name, 0), C.free)
		if dname == nil then return nil end
		knot.knot_dname_to_lower(dname);
	...
	rr2str = rr2str,
	str2ip = function (ip)
		local family = libkres.kr_straddr_family(ip)
		local ret = C.inet_pton(family, ip, addr_buf)
		if ret ~= 1 then return nil end
		return ffi.string(addr_buf, libkres.kr_family_len(family))
	end,
